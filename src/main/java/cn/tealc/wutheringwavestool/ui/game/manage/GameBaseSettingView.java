...
public class GameBaseSettingView implements FxmlView<GameBaseSettingViewModel>, Initializable {
    ...
    @FXML
    private Button switchToGlobalBtn;
    @FXML
    private Button switchToOfficialBtn;
    @FXML
    private Label currentServerLabel;
    @FXML
    private Label diffStatusLabel;
    @FXML
    ...
    @FXML
    private ProgressBar resourceProgress;
    @FXML
    private Label resourceMessage;
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        viewModel.init();
        initGameAsset();
        initStartParam();
        initServerSwitchPanel();
    }
...
/*    void setSelectedGameType(ActionEvent event) {
    ...
    private void initServerSwitchPanel(){
        currentServerLabel.textProperty().bind(viewModel.currentServerNameProperty());
        diffStatusLabel.textProperty().bind(viewModel.diffStatusProperty());
        resourceProgress.progressProperty().bind(viewModel.syncProgressProperty());
        resourceMessage.textProperty().bind(viewModel.syncMessageProperty());
        syncBtn.disableProperty().bind(viewModel.syncRunningProperty());
        predownloadBtn.disableProperty().bind(viewModel.syncRunningProperty());
        applyBtn.disableProperty().bind(viewModel.applyEnabledProperty().not().or(viewModel.syncRunningProperty()));
    }

    @FXML
    void switchServerToGlobal(ActionEvent event) {
        viewModel.switchToGlobalServer();
    }

    @FXML
    void switchServerToOfficial(ActionEvent event) {
        viewModel.switchToOfficialServer();
    }
    ...
    @FXML
    void startPredownload(ActionEvent event) {
        viewModel.startPredownload();
    }

    @FXML
    void applyPredownload(ActionEvent event) {
        viewModel.applyPredownload();
    }
